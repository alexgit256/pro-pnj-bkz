...
struct Params{
    ...
    int cost_model = 3; 
    int compute_jub = 5; //Compute jump upper bound in 1:pessimistic d4f(B); 2: optimistic d4f(B); 3: d4f(beta)/2; 4: min(d4f(beta),0.1beta); 5: min{aisaccsd4f(slope),theo1d4f(beta)}
    double jub_ratio = 0.08; //Set a jump upper bound, where jump<=jub_ratio * beta.
    ...
    int sim_d4f = 1; //1:the previous correctness for beta with jump>1 before 2024.1.31; 2: the new correctness for beta with jump>1 after 2024.1.3(more refined)
    ...
    int practical_pump_d4f = 3; //the dim4free function for last pump in prectical cost mode. 1: theo d4f1; 2: theo d4f2; 3: d4f in default g6k; 4: optimistic d4f(B); 5: accs_d4f(slope)
};
