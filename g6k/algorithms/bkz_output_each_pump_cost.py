...
def pump_n_jump_bkz_tour(g6k, tracer, blocksize, jump=1,
    ...
    for (kappa, beta, f) in indices:
        ...
        T0 = time.time()
        _, max_RAM_cost =  pump(g6k, tracer, kappa, beta, f, **pump_params)
    ...
    T0 = time.time()
    _, max_RAM_cost = pump(g6k, tracer, d-(blocksize-dim4free), blocksize-dim4free, 0, **pump_params)
    # rr = [g6k.M.get_r(_,_) for _ in range(g6k.M.B.ncols)]
    # File.write(str(rr))
    # File.write('\n')
    T_pump = time.time()-T0
    ...
    # print(T_pumps)
    # File.close()
    return T_pumps, indices + [(d-(blocksize-dim4free), blocksize-dim4free, 0)],max_RAM_cost
