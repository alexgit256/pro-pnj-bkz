...
def read_file(n,alpha_,jump,Pumpdown,Blocksize,Tours,test_number,d):
    dir = "pro-bkz-tests/gs-lengths-simulator/"+"n=%d,alpha=%s,jump=%d,Pumpdown=%s,Blocksize=%d,Tours=%dd=%d,/" %(n,alpha_,jump,Pumpdown,Blocksize,Tours,d)

    log_GS_lengths,GS_lengths = read_gs_lengths(dir+"rr_set,test_number=%d.txt" % (test_number))
...
def show_gs_slope_figure4(dir,log_gs_length,sim_log_gs_lengths,n,dimension,alpha_,square_error1,Blocksize,Jump,N,test_number):
    ...
    t = 0
    plt.scatter([_+1 for _ in range(t,dimension)],[math.exp(sim_log_gs_lengths[_])/math.exp(log_gs_length[_]) for _ in range(t,dimension) ],s=12,marker="*",c='k',edgecolors='k')
    plt.ylim(0.9,1.1)
...
def show_gs_slope_figure5(dir,log_gs_lengths,reduc_log_gs_lengths, sim_log_gs_lengths, n,dimension,alpha_,square_error1,Blocksize,Jump,N,test_number,first_num):
    plt.figure(figsize=(15, 10), dpi=100)

    t = 0
    plt.scatter([_+1 for _ in range(t,dimension)],[log_gs_lengths[_] for _ in range(t,dimension) ],marker="^",c='none',edgecolors='brown')
    plt.scatter([_+1 for _ in range(t,len(reduc_log_gs_lengths)) ][0:first_num],[reduc_log_gs_lengths[_] for _ in range(t,len(reduc_log_gs_lengths)) ][0:first_num],marker="^",c='none',edgecolors='black')
    plt.scatter([_+1 for _ in range(t,len(sim_log_gs_lengths)) ][0:first_num],[sim_log_gs_lengths[_] for _ in range(t,len(sim_log_gs_lengths)) ][0:first_num],marker="x",color='blue')
    plt.xlabel("Index", fontsize=24)
    plt.ylabel("log GS norm", fontsize=24)
    plt.legend(['Input GS norm','GS norm after reduction', 'GS norm after Simulation'], fontsize=24)
    #plt.title("n=%d, alpha = %s, dimension = %d, Blocksize = %d, Jump = %d, #Current Tours= %d, prediction error = %f" %(n,alpha_,dimension,Blocksize,Jump,N+1,square_error1))
    plt.title("Dimension = %d, Blocksize = %d, Jump = %d, #Current Tours= %d" %(dimension,Blocksize,Jump,N+1), fontsize=20)
    plt.tick_params(labelsize=22)

    plt.savefig(dir+"GS norm after reduction figure/n=%d, alpha = %s, dimension = %d, Blocksize = %d, Jump = %d, #Current Tours= %d, former num = %d.png" %(n,alpha_,dimension,Blocksize,Jump,N+1, first_num))
#    plt.show()
    plt.close()
...
def ratio_txt(dir,log_gs_length,sim_log_gs_lengths,n,dimension,alpha_,square_error1,Blocksize,Jump,N,test_number):
    ...
    for _ in range(0,dimension):
        f.write(str( math.exp(sim_log_gs_lengths[_])/math.exp(log_gs_length[_]) )+' ')
    f.close()
...
if __name__ == "__main__":
    ...
    Blocksize, jump, Tours, test_number = 100, 6, 13, 1
    ...
    def Theory1_d4f(beta):
        return math.floor(beta*log(4/3)/log(beta/(2*pi*exp(1))))
    
    #Theory optimistic d4f value
    def Theory2_d4f(beta):
        return math.floor(beta*log(4/3)/log(beta/(2*pi)))

    def accs_2023_d4f(slope):
        #return math.floor(log(sqrt(4/3))/log(exp(1)**(-1*slope/4)))
        return math.floor(log(sqrt(4/3))/(-1*slope/4))
                          
    #d4f_value = "d4f value based on the quality of current lattice basis"
    ...
    def d4f_gap(beta):
        if beta<40:
            ...
            gap = (beta-40)/2-Theory1_d4f(beta)
        elif beta>0:
            gap = (11.5+0.075*beta)-Theory1_d4f(beta)
        return round(gap)
    ...
    def d4f_gap2(beta):
        if beta<40:
            ...
            gap = (beta-40)/2-Theory2_d4f(beta)
        elif beta>0:
            gap = (11.5+0.075*beta)-Theory2_d4f(beta)
        return round(gap)
    
    def d4f_gap_accs(beta,slope):
        if beta<40:
            gap = 0
        elif beta<75:
            gap = (beta-40)/2-min(Theory1_d4f(beta),accs_2023_d4f(slope))
        elif beta>0:
            gap = (11.5+0.075*beta)-min(Theory1_d4f(beta),accs_2023_d4f(slope)-1)
        return round(gap)
    ...
    #log_GS_lengths we get from g6k are 2*log||bi*||, so we do following pre-processingg: 
    ...
    # print(N)
    # print(log_GS_lengths[0][0], log_GS_lengths[N][0],GS_lengths[0][0],GS_lengths[N][0])
    
    sim_log_gs_lengths = copy.deepcopy(pnjBKZ_simulator(log_GS_lengths[0], Sim_Blocksizes[0], 1, d, jump))
    for i in range(2*jump+1):
        show_gs_slope_figure5(dir,log_GS_lengths[0],log_GS_lengths[1],sim_log_gs_lengths, n,d,alpha_,square_error1,Blocksizes[0],jump,0,test_number,i)
    show_gs_slope_figure5(dir,log_GS_lengths[0],log_GS_lengths[1],sim_log_gs_lengths, n,d,alpha_,square_error1,Blocksizes[0],jump,0,test_number,d)
