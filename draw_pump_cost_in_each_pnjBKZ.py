...
#        ax.plot([0,len(pump_costs)-1],[MinPumpCost , MinPumpCost ],label="MinPumpCost = %.2f sec"%(MinPumpCost),color = "brown",linestyle="--")
        ...
        ax.set_title(r"d = %d, blocksize = %d, jump = %d, f = %d, exf = 12, sieve_dim = %d"%(d,blocksize,jump,f,blocksize-f), fontsize="8")
        ax.set(xlabel='index: i')
        ax.set(ylabel='cost (sec)')
        fig.savefig("each_pump_cost_in_pnjbkz/auxiliary_fig/EachPumpCost-dd-in-PnJBKZ(%d,%d,%d).png"%(d,blocksize,jump))
        plt.close()
...
for blocksize in range(51,120,2):
    ...
    for jump in range(1, dim4free_wrapper(default_dim4free_fun, 180)+1):
        ...
        if(flag):
            ...
            ax.autoscale(tight=False)
            ax.set_title(r"blocksize = %d, jump = %d, f = %d, exf = 12, sieve_dim = %d"%(blocksize,jump,f,blocksize-f), fontsize="8")
